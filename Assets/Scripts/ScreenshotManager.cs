...
public class ScreenshotManager : MonoBehaviour {
	...
	public int starBonus = 5;
	public GameObject shared;
	public GameObject unshared;
	...
	public void TakeAndShareScreenshot() {
		Debug.Log ("Take and share screenshot");
		...
		if (!isProcessing) { 
			...
			if (!wasShared) {
				PlayerPrefs.SetInt("Stars", PlayerPrefs.GetInt("Stars") + starBonus);
				if (SceneManager.GetActiveScene().name != "Title") {
					indicatorText.text = (int.Parse(indicatorText.text) + starBonus).ToString();
				}
				wasShared = true;
			}
	...
	#if UNITY_ANDROID
	public IEnumerator ShareAndroid()
	{
		
		//Debug.Log ("Share IENumerator");
		isProcessing = true;
		...
		if(!Application.isEditor)
		{
			...
			AndroidJavaObject jChooser = intentClass.CallStatic<AndroidJavaObject>("createChooser", intentObject, "PLEASE SELECT AN APP");
